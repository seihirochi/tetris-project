...
class NNTrainerController(Controller):
    ...
    def train(self, env: Env, episodes=1):
        ...
        for _ in range(episodes):
            ...
            while not done:
                ...
                if info["is_lower"]:
                    self.lower_experience_buffer.add((state, action, reward, next_state, done))
                else:
                    self.upper_experience_buffer.add((state, action, reward, next_state, done))
    ...
    def learn(self, batch_size=128, epochs=8):
        ...
        if self.lower_experience_buffer.len() < batch_size // 2 or self.upper_experience_buffer.len() < batch_size - batch_size // 2:
        ...
        all_targets = self.model.predict(np.concatenate([states, next_states]), batch_size=(batch_size*2))
        ...
        print(f"Action max value for the first sample in batch after learning: {targets[idx]}\n")
