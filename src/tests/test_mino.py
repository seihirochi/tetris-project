...
class TestMino:
    ...
    def test_failcreation__with_invalid_shape(self):
        with pytest.raises(ValueError):
            Mino(1, np.array([[0, 0, 1], [0, 0, 1]]), (0, 0))

    def test_failcreation__with_invalid_fulcrum_shape(self):
        with pytest.raises(ValueError):
            Mino(1, mock_mino_shape, (0, 1, 2))

    def test_failcreation__with_invalid_fulcrum_position(self):
        with pytest.raises(ValueError):
            Mino(1, mock_mino_shape, (3, 3))

    def test_failcreation__with_empty_char(self):
        with pytest.raises(ValueError):
            Mino(1, mock_mino_shape, (0, 0), "")
